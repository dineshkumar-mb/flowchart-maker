...
const getShapeStyle = (shapeType, customColor) => {
  const baseStyle = {
...
function FlowchartEditor() {
  ...
  const handleVoiceCommand = async (commandText) => {
    ...
    try {
      ...
      let shapeData;
      try {
        shapeData = JSON.parse(resultText);
      } catch {
        const match = resultText.match(/\{[\s\S]*?\}/);
        if (match) {
          shapeData = JSON.parse(match[0]);
        } else {
          alert("❌ No valid JSON structure found.");
          return;
        }
      }

      if (shapeData.action === 'add') {
        addShapeNode(shapeData.shape.toLowerCase(), shapeData.label);
      } else if (shapeData.action === 'move') {
        const { targetShape, label, x, y } = shapeData;
        setNodes((nds) =>
          nds.map((node) =>
            node.data.shapeType.toLowerCase() === targetShape.toLowerCase() &&
        ...
        const { targetShape, label, color } = shapeData;
        setNodes((nds) =>
          nds.map((node) =>
            node.data.shapeType.toLowerCase() === targetShape.toLowerCase() &&
            node.data.label === label
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', width: '100vw' }}>
      <div style={{ width: '240px', padding: '10px', borderRight: '1px solid #eee' }}>
        <h3>Add Shape</h3>
        {['circle', 'square', 'rectangle', 'diamond', 'hexagon'].map((shape) => (
          <button
            key={shape}
            onClick={() => addShapeNode(shape)}
            style={{ marginBottom: '10px', width: '100%' }}
          >
            Add {shape.charAt(0).toUpperCase() + shape.slice(1)}
          </button>
        ))}
        <hr />
        <button
...
//       }}
//       onMouseEnter={(e) =>
//         (e.currentTarget.style.boxShadow = '0 0 10px rgba(242, 236, 236, 0.91)')
//       }
//       onMouseLeave={(e) => (e.currentTarget.style.boxShadow = 'none')}
//     >
//       <Handle type="target" position={Position.Top} style={{ background: '#555' }} />
